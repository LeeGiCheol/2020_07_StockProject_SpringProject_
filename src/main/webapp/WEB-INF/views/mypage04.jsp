						...
						<div class="board-type">
							<div class="board-free-nav">
								<form id="form" class="board-list-top policy-in" action='/board/free'>
									<p class="pc-only">
										<input type="radio" class="ordeby" id="orderby1" name="orderby" value="new"
											checked=""><label for="orderby1" class="new-board">거래 알림</label>
							...
							<div class="tab-content" id="pills-tabContent">
								...
								$(document).ready(function(){
									 $("#orderby2").click(function(){
										 $("#a_title").html("<tr><th class='no' scope='col'>글번호</th><th class='title' scope='col'>내용</th></tr>");
										 console.log("클릭");
										 $("#a_contents").html('<c:forEach items="${commentNotice}" var="list"><tr><td class="first" title="글번호"><c:out value="${list.pno}" /></td><td title="내용"><c:out value="${list.title}" /> 글에 댓글이 달렸습니다.</td>	</tr></c:forEach>');
									 })
									  $("#orderby1").click(function(){
										 $("#a_title").html('<tr><th class="no" scope="col">N0</th><th class="title" scope="col">종목명</th><th class="writer" scope="col">단가</th><th class="views" scope="col">수량</th><th class="likes" scope="col">거래일시</th><th class="date" scope="col">상태 </th></tr>');
										 console.log("클릭");
										 $("#a_contents").html('<tr><c:forEach items="${modifiedNotice}" var="list"><td class="first" title="주문번호"><c:out value="${list.tno}" /></td><td title="종목명"><c:out value="${list.stockName}" /></td><td title="주문단가"><c:out value="${list.tprice}" /></td><td title="주문수량"><c:out value="${list.quantity}" /></td><td title="거래일시"><fmt:formatDate value="${list.tdatetime}" pattern="yyyy-MM-dd HH:mm:ss" /></td><td title="상태"><c:out value="${list.category}" /></td></tr></c:forEach>');
									 })
										...
										<nav aria-label="..." class="pagination">
											<ul class="pagination">
												...
												<c:if test="${boardPage.nowPage != 1}">
													<!-- << 버튼 -->
													<li>
														<a class="page-link" href="/board/free?bnowPage=1" tabindex="-1"
															aria-disabled="true">
												...
												<c:forEach begin="${boardPage.startPage }" end="${boardPage.endPage }"
													...
													<c:choose>
														...
														<c:when test="${p != boardPage.nowPage}">
															<li class="page-item">
																<a class="page-link"
																	href="/board/free?bnowPage=${p}">${p}</a>
															</li>
														</c:when>
													</c:choose>
												</c:forEach>
							...
							data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">제목</a> -->
											<select class="dropdown-toggle-board" name="searchStyle">
												<option class="nav-link dropdown-toggle board-item" id="dropdown01"
													...
													value="" <c:if test='${searchStyle eq ""}'>selected</c:if>>전체
												</option>
